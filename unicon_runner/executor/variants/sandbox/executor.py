...
class SandboxExecutor(Executor):
    ...
    async def _execute(
        ...
        proc = await asyncio.create_subprocess_shell(
            f"SANDBOX=1 SANDBOX_LEVEL=1 QUIET_MODE=1 {self.CONTY} --bind {os.path.abspath(folder_path)} ~/{folder_path} --ro-bind {os.path.abspath(self.RUN_SCRIPT)} ~/{self.RUN_SCRIPT} ./{self.RUN_SCRIPT} {folder_path} {self.CODE_FOLDER_NAME}/{request.entrypoint} {request.environment.memory_limit * 1024} {request.environment.time_limit}",
